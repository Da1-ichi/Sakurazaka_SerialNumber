...
/** 連続スキャン時の軽量パス（処理時間優先） */
const QUICK_PASSES: PassConfig[] = [
  { thresholdOffset: 0 },
  { thresholdOffset: 25 },
];

/** 手動シャッター時の高精度パス（精度優先） */
const ACCURATE_PASSES: PassConfig[] = [
  { thresholdOffset: -20 },
  { thresholdOffset: 0 },
...
function scoreCandidate(code: string, occurrenceCount = 0, isVoted = false): number {
  let score = 0;
 ...
 */
function extractRawSerials(text: string): string[] {
 ...
 */
function voteByPosition(rawCandidates: string[]): string | null {
...
/**
 * 多パスの生候補リストから、曖昧文字展開＋スコアリングで最終候補リストを生成する。
 */
function aggregateCandidates(rawCandidatesAllPasses: string[]): string[] {
...
export default function SerialReaderPrototype() {
   ...
   * 3倍に拡大 → コントラスト強調 → (オプション: シャープニング) → 二値化 → (オプション: モルフォロジー)。
   *
   * @param passCfg しきい値オフセットや追加処理を指定する。省略時は通常パス相当。
   */
  function captureGuideArea(passCfg?: PassConfig): string | null {
    ...
    // Step 4: モルフォロジー演算
    //   thicken: 黒（文字）を膨張 → 細い線が消えるのを防ぐ
    //   thin:    黒を収縮 → 太すぎる文字を細く（潰れ防止）
    let finalBuf = binary;
  ...
  /**
   * OCR 実行。複数の前処理パスで実行し、結果を位置投票で集約する。
   * - 連続スキャン (autoSave): 2 パス（高速モード）
   * - 手動シャッター: 5 パス（高精度モード、しきい値変更・シャープニング・モルフォロジー）
   */
  async function readSerial(options?: { autoSave?: boolean }) {
    try {
      ...
      setStatusText(
        isAuto
          ? "連続スキャン実行中..."
          : `OCR実行中... (高精度モード ${passes.length}パス)`
      );
      setRawText("");
      setCandidates([]);
      ...
      for (let i = 0; i < passes.length; i++) {
        ...
        if (!isAuto) {
          setStatusText(`OCR実行中... パス ${i + 1}/${passes.length}`);
        }
        ...
        // 各パスから 13 桁の生候補を抽出
        const rawSerials = extractRawSerials(text);
        allRawCandidates.push(...rawSerials);
      }
      ...
      if (isAuto && found[0]) {
        ...
        // 何パスで同じ生候補が出たかを表示してユーザーに信頼度を伝える
        const occurrence = new Map<string, number>();
        for (const c of allRawCandidates) {
          occurrence.set(c, (occurrence.get(c) ?? 0) + 1);
        }
        const topOccurrence = occurrence.get(found[0] ?? "") ?? 0;
        const suffixHint = `末尾${CURRENT_SINGLE_SUFFIX}優先`;
        const matchHint =
          topOccurrence >= 2 ? `・${topOccurrence}/${passes.length}パス一致` : "";

        setStatusText(
          found.length > 0
            ? `候補 ${found.length} 件（${suffixHint}${matchHint}）`
            : "候補なし。位置を合わせ直してください。"
        );
      }
